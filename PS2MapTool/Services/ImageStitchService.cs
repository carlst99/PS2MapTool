...
namespace PS2MapTool.Services
{
    ...
    public class ImageStitchService : IImageStitchService
    {
        ...
        private readonly ITileProcessorService _tileProcessor;

        /// <summary>
        /// Initialises a new instance of the <see cref="ImageStitchService"/> class.
        /// </summary>
        /// <param name="tileProcessor">The tile processor to use.</param>
        public ImageStitchService(ITileProcessorService tileProcessor)
        {
            _tileProcessor = tileProcessor;
        }

        /// <inheritdoc />
        public virtual async Task<Image<Rgba32>> StitchTiles(IList<TileInfo> tiles, CancellationToken ct = default)
        {
            ...
            {
                using Image tileImage = await _tileProcessor.Load(tile.DataSource, ct).ConfigureAwait(false);
